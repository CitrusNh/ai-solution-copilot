...
**作品集材料：** [系统架构](docs/architecture.md) · [项目经历与简历描述](docs/portfolio-package.md) · [演示脚本](docs/demo-script.md) · [面试讲解](docs/interview-guide.md) · [最终验证报告](eval/final-report.md)
...
这是一套可以公开演示的本地规则 MVP，不是生产系统：当前不连接大模型，资料仅用于当前会话，公网 Demo 只应使用项目内演示资料或非敏感文件。真实企业用户反馈尚未收集，不把开发者自测当作市场验证。
...
如果终端显示 Streamlit 的本地地址，终端进程需要保持运行；关闭终端会停止本地网页服务。公网 Demo 不需要用户电脑保持终端运行。
