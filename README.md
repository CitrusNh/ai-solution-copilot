...
- 10 道冻结开发评测题 10/10 通过，33 项自动化测试通过。

聊天增强是可选能力。当前仓库已完成接口、约束校验、失败降级和模拟 API 测试；只有部署者在 Secrets 中配置有聊天模型权限的 Key 和模型名后，页面开关才会启用。当前 APINebula Key 只返回图片模型，因此不能把公网 Demo 描述为已经完成真实聊天调用。
