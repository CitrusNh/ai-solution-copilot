...
- `packages.txt` 列出 OpenCV/RapidOCR 在 Linux 上需要的系统运行库。
...
3. OCR 初始化失败：确认 `packages.txt` 已安装 `libgl1` 和 `libglib2.0-0t64`；当前 Streamlit Cloud 的 Debian 环境不要使用旧包名 `libglib2.0-0`，再查看云端异常日志。
